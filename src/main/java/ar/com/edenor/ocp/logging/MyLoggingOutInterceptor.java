...
public class MyLoggingOutInterceptor extends AbstractLoggingInterceptor {
    ...
    public MyLoggingOutInterceptor(PrintWriter writer) {
        this((LogEventSender)(new PrintWriterEventSender(writer)));
    }

    public MyLoggingOutInterceptor(LogEventSender sender) {
        super("pre-stream", sender);
        this.addBefore(StaxOutInterceptor.class.getName());
    }

    public void handleMessage(Message message) throws Fault {
        maskSensitiveHelper.addSensitiveElementNames(new HashSet<>(Arrays.asList("password")));
        if (!isLoggingDisabledNow(message)) {
            ...
            if (os != null) {
                MyLoggingOutInterceptor.LoggingCallback callback = new MyLoggingOutInterceptor.LoggingCallback(this.sender, message, os, this.limit);
                ...
                if (iowriter != null) {
                    message.setContent(Writer.class, new MyLoggingOutInterceptor.LogEventSendingWriter(this.sender, message, iowriter, this.limit));
                }
            }
    ...
    public class LoggingCallback implements CachedOutputStreamCallback {
        ...
        public void onClose(CachedOutputStream cos) {
            ...
            if (MyLoggingOutInterceptor.this.shouldLogContent(event)) {
        ...
        private void copyPayload(CachedOutputStream cos, LogEvent event) {
            try {
                ...
                final String maskedContent = maskSensitiveHelper.maskSensitiveElements(message, payload.toString());
                event.setPayload(removeUsernameToken(maskedContent));
                event.setPayload(maskedContent);
                setTypeInterfaceToMDC(message);
                evaluateResponseMessage(message,maskedContent);
                boolean isTruncated = cos.size() > (long)MyLoggingOutInterceptor.this.limit && MyLoggingOutInterceptor.this.limit != -1;
    ...
    private class LogEventSendingWriter extends FilterWriter {
        ...
        public void close() throws IOException {
            ...
            String payload = MyLoggingOutInterceptor.this.shouldLogContent(event) ? this.getPayload(event, w2) : "--- Content suppressed ---";
            final String maskedContent = maskSensitiveHelper.maskSensitiveElements(message, payload);
            event.setPayload(removeUsernameToken(maskedContent));
            event.setPayload(maskedContent);
            removeHeadersAuthorization(event);
            this.sender.send(event);
    ...
    private void evaluateResponseMessage(Message message , String payload) {
        String errorCode =  MDC.get(LoggingConstants.ERROR_CODE);
        String errorDescription =  MDC.get(LoggingConstants.ERROR_DESCRIPTION);
...
//        MDC.remove(LoggingConstants.ERROR_DESCRIPTION);
        ...
        if (Objects.nonNull(responseCode) && (responseCode.intValue() <200 || responseCode.intValue()  > 299)) {
            if (Objects.nonNull(payload)) {
                if (Objects.isNull(errorCode) || Objects.isNull(errorDescription) || errorCode.isEmpty() || errorDescription.isEmpty()) {
                    errorCode = errorCodeHelper.findValue(LoggingConstants.ERROR_CODE_ATTRIBUTE_NAME,payload, MaskSensitiveHelper.FindTypeValue.STRING);
                    errorDescription = errorCodeHelper.findValue(LoggingConstants.ERROR_DESCRIPTION_ATTRIBUTE_NAME,payload, MaskSensitiveHelper.FindTypeValue.STRING);
                }
